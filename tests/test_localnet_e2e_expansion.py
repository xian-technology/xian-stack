...
class LocalnetE2EExpansionTests(unittest.TestCase):
    ...
    def test_node_report_snapshot_does_not_poll_after_deadline(self) -> None:
        args = localnet_e2e.build_parser().parse_args(["--rpc-timeout-seconds", "30"])
        with tempfile.TemporaryDirectory() as tmpdir:
            args.resume_dir = tmpdir
            runner = localnet_e2e.E2ERunner(args)
        ...
        lagging_report = {
            "ok": False,
            "checks": {"height_spread_within_tolerance": False},
            "totals": {"height_spread": 3},
            "errors": [],
            "nodes": [
                {"moniker": "node-0", "height": 42, "catching_up": False},
        ...
        with (
            ...
            patch.object(
                localnet_e2e.asyncio,
                "sleep",
                AsyncMock(side_effect=advance_clock),
            ) as sleep,
            patch.object(localnet_e2e.time, "monotonic", side_effect=lambda: now[0]),
        ):
            with self.assertRaisesRegex(localnet_e2e.E2EError, '"height_spread": 3'):
                asyncio.run(runner.collect_node_report_snapshot(timeout_seconds=1.0))
